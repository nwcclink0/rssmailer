...
async fn post_login_account(
    ...
    let pool = context;
    if json.provider == AuthProvider::RssmailerAuth as u32 {
        let validate =
...
#[post("/account/send_verify_email_code")]
        ...
        match fetch_account_info(&pool, uuid).await {
            Ok(account_info) => {
                ...
                let time1 = SystemTime::now()
                    ...
                    .unwrap()
                    .as_secs();
                let verify_code = auth.verify(info.1, 30, time1);
                if verify_code {
...
async fn send_verify_email_link(account: AccountInfo, code: u32) -> Result<()> {
    ...
    let smtp_username = "ytwei681@gmail.com".to_string();
    let smtp_password = "nxphozhhlowshigg".to_string();
    let link = format!(
            ...
            MultiPart::alternative()
                .singlepart(
                    SinglePart::builder()
                        ...
                        .body(String::from("RssMailer from ArcSparrow")),
                )
                .singlepart(
                    SinglePart::builder()
